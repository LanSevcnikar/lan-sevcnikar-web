...
function Portfolio() {
  const { t } = useLanguage()
  const groups = { [t('languages')]: ['Python', 'JavaScript', 'Java', 'SQL'], [t('tools')]: ['PyTorch', 'pandas', 'Git', 'Firebase'], [t('focus')]: ['Machine learning', 'Data science', 'Graphics'], [t('interests')]: ['Visualization', 'Algorithms', 'Web'] }
  return <section className="page section"><SectionLabel>{t('portfolio')}</SectionLabel><h1>{t('portfolioHeading')}</h1><p className="lede">{t('portfolioIntro')}</p><div className="skills">{Object.entries(groups).map(([name, items]) => <div key={name}><SectionLabel>{name}</SectionLabel>{items.map(item => <span key={item}>{item}</span>)}</div>)}</div><div className="info-grid"><div><SectionLabel>{t('education')}</SectionLabel><h3>{t('university')}</h3><p>{t('bachelorDegree')}</p><p>{t('masterDegree')}</p></div><div><SectionLabel>{t('experience')}</SectionLabel><h3>Flare · Abelium</h3><p>{t('experienceBody')}</p></div></div><div className="section-heading cv-heading"><div><SectionLabel>{t('cv')}</SectionLabel><h2>{t('fullCv')}</h2></div><div className="actions"><a className="button button-primary" href="/CV.pdf" target="_blank" rel="noreferrer">{t('openPdf')}</a><a className="button" href="mailto:sevcnikar.lan@gmail.com">{t('emailMe')}</a></div></div><object className="cv" data="/CV.pdf#toolbar=0&view=FitH" type="application/pdf"><a href="/CV.pdf">{t('openCvPdf')}</a></object></section>
}
